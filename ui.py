...
class SharkLiteUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("SharkLite Configurator v1.0")
    ...
    def build_ui(self):
        ...
        ttk.Button(frame,text="Save",command=self.save_key)\
            .grid(row=3,column=2,padx=5)

        ttk.Separator(frame).grid(row=4,column=0,columnspan=3,sticky="ew",pady=8)

        ttk.Label(frame,text="Button").grid(row=5,column=0,sticky="w")
        ttk.Label(frame,textvariable=self.buttonVar).grid(row=5,column=1,sticky="w")

        ttk.Separator(frame).grid(row=6,column=0,columnspan=3,sticky="ew",pady=8)
        ...
        self.console.grid(row=9,column=0,columnspan=3)
